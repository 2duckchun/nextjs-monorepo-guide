...
- 서울특별시는 서울특별시에 해당하는 지역구에 자원(돈)을 배분해줄 것이다. 각 구마다 서로 자원(돈)을 빌려줄수도 있을 것이다.

- 모노레포는 최상단의 워크스페이스로부터 하위 워크스페이스로 자원(의존성)을 배분해줄 수 있고, 동등한 레벨의 워크스페이스 사이에서도 자원(의존성)을 배분할 수 있다.

그래서 그런것일까. 서울특별시 지도와 모노레포를 표현한 그림은 비슷한 면이 있다.
...
추상적인 부분은 추상적으로만 이해하자.

## 키워드

모노레포에서 가장 중요한 키워드는 아래와 같다

1. 패키지매니저(package manager)
2. 워크스페이스(workspace, 작업공간)
3. 빌드 툴(build tool, build system)

모노레포는 하나의 레포에서 여러개의 프로젝트를 관리한다는 개념이다. 이를 위해서는 **워크스페이스**라는 개념이 필요한데, 워크스페이스의 개념을 **패키지매니저**에서 제공한다.

패키지매니저로 워크스페이스를 선언해서 구성한 모노레포를 좀 더 효율적으로 굴러갈 수 있게 지원해주는 툴은 **빌드 툴**이다. 

위의 세 키워드가 모노레포의 구성의 핵심이다. 개념만 차근차근 알아보자.
