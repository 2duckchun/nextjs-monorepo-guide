...
사실 내 상황에 100% 필요한 모노레포 구성 레퍼런스는 찾기가 어려운 것이 맞다. 모노레포는 사랑과 우정, 용기같은 추상적인 개념이므로 세부 구현 방법은 사용자마다 달라질 수밖에 없다. **즉, 각자의 니즈에 맞게 커스터마이징 할 수밖에 없다.**

하여 이 가이드는 모노레포를 자유자재로 구현하기 위해 개발자가 알아야하는 키워드들과 실 구현 코드를 바탕으로 작성되었다.
